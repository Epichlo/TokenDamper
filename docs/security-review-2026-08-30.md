...
| **V-02** ✅ **FIXED** | `Origin: null` bypasses the browser-origin refusal entirely | **Low** | Gateway (`exec`) | `src/gateway/server.ts:335` | The guard reads `originHeader !== 'null'`, so the one Origin value a browser sends when it is *most* sandboxed is the one value exempted. Measured: `Origin: https://evil.example` → **403, no session**; `Origin: null` → **200, session created**. A sandboxed iframe, a `data:` URL and some redirect chains all produce `Origin: null`, so a page can reach the loopback gateway with a simple `text/plain` POST that needs no preflight. It **cannot** get a request forwarded upstream — `authorization` is not CORS-safelisted, so supplying it forces a preflight the server answers `405` with no `Access-Control-*` — but it does reach session handling, which is where V-02 meets the F-01 residual below. | Treat `null` as a *foreign* origin rather than an absent one. An absent `Origin` header is the non-browser client; the literal string `null` is a browser declining to name itself, which is not the same thing. |

**Both are fixed, and so is the residual §10.2 re-values.** Landed against v1.7.3 with the tests
that would have caught them:

- **V-01** — `findMemberValue` is last-match-wins now, matching `JSON.parse`. Verified end to end:
  the forwarded body keeps the shadowed first value **and** elides the real duplicate, so the block
  appears once rather than twice. Both halves matter — the old behaviour destroyed one value *and*
  saved nothing.
- **V-02** — `null` is treated as a foreign origin. `Origin: null` is refused 403 with no session
  created; an *absent* `Origin`, which is the non-browser client, still passes.
- **F-01's residual** — the credential check is hoisted above `getOrCreateSession`, so an
  unauthenticated request creates nothing. Measured: 20 unauthenticated requests leave
  `sessionCount` at **0**, so the eviction primitive is gone for the caller V-02 reaches. A
  rejected method no longer creates one either.

**One regression was introduced and caught by an existing test.** Hoisting the 401 removed, as a
side effect, the branch where `allowMissingUpstreamCredentials` answers locally instead of
forwarding — `gateway-response-headers.test.ts` failed on it immediately. Restored explicitly in
both routes. That is the M8 seam's own regression test doing exactly what it was written for.
...
### 12.8 Remediation of S-01 and S-02

- **Date:** 2026-09-07, the same day and a separate change. **DECISIONS §73** carries the reasoning;
  this is the record.
- **Written test-first.** Both defects were reproduced as failing unit cases before either fix was
  written — S-01 as `expected 20 to be +0` and `expected 100 to be +0`, S-02 as
  `expected [ Array(4) ] to have a length of 3 but got 4` and
  `expected [ Array(9) ] to not include '==> security_policy.py <=='`. Nine cases now stand in
  `test/unit/security-review-findings.test.ts`, including two that fail if the fix over-reaches:
  a credentialed request on an API route must still get a session, and the fallback stream must
  still carry each file's original bytes.

| | Fixed by | Verified |
|---|---|---|
| **S-01** | The session is opened inside the two API branches (`openSession()`), so no other route can mint one; the 404 reports none. `ProxyRequestResult.session` was already optional and is read by nobody. | Same attacker page, same 400 no-cors GETs, against a rebuilt gateway: **`sessionCount` 1**, and it is the victim's, still holding its content — against 100 and evicted before. Unit: unknown endpoint ×20 → 0; `x-session-id` ×120 → 0. |
| **S-02** | The escaping is now an exported `escapeDelimiterLabel` in `core/render`, called by `itemLabel` and by `renderFallbackBytes`. | R-14 re-run on ext4: the fallback stream emits **one header per file, all absolute**, and the crafted name appears escaped on a single line. `ALLOW_INSECURE_TLS = True` is no longer a line of its own. |

**Suite:** 99 files / **927 passed, 2 skipped** (the two F-04 mode assertions, POSIX-only).
Typecheck, lint and build clean.

**No corpus run, for the reason 12.6 already gives** — S-01 is off the optimize route, and S-02
